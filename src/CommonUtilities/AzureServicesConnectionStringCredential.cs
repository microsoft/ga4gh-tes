...
namespace CommonUtilities
{
    ...
    public class AzureServicesConnectionStringCredentialOptions : Azure.Identity.TokenCredentialOptions
    {
        ...
        private AzureServicesConnectionStringCredentialOptions(ArmEnvironmentEndpoints armEndpoints)
        {
            ...
            Resource = armEndpoints.ResourceManager;

            if (string.IsNullOrWhiteSpace(TenantId))
            {
                TenantId = armEndpoints.Tenant;
            }
        }
    ...
    internal partial class AzureServicesConnectionStringCredentialFactory
    {
        ...
        [GeneratedRegex("([\\s;]*(?![\\s;])(?<key>([^=\\s\\p{Cc}]|\\s+[^=\\s\\p{Cc}]|\\s+==|==)+)\\s*=(?!=)\\s*(?<value>(\"([^\"\0]|\"\")*\")|('([^'\0]|'')*')|((?![\"'\\s])([^;\\s\\p{Cc}]|\\s+[^;\\s\\p{Cc}])*(?<![\"'])))(\\s*)(;|[\0\\s]*$))*[\\s;]*[\0\\s]*", RegexOptions.ExplicitCapture | RegexOptions.Compiled)]
        private static partial Regex ConnectionStringRegexGenerated();
        private static readonly Regex ConnectionStringRegex = ConnectionStringRegexGenerated();
