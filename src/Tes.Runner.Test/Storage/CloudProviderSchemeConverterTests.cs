...
namespace Tes.Runner.Test.Storage
{
    ...
    public class CloudProviderSchemeConverterTests
    {
        ...
        public async Task TransformUrlWithStrategyAsync_URIProvided_MatchesExpected(string sourceUri, string expectedUri)
        {
            var provider = new CloudProviderSchemeConverter();

            var result = await provider.TransformUrlWithStrategyAsync(sourceUri, BlobSasPermissions.All);
