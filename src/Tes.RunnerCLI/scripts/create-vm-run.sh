...
# Script to create an Azure VM with randomly generated names for resource group and VM,
# set a resource tag, and handle script cancellation
...
# Function to generate a random lowercase string of letters
generate_random_name() {
    cat /dev/urandom | tr -dc 'a-z' | fold -w ${1:-10} | head -n 1
}

# Generate random names for resource group and VM
...
# Set the provided subscription ID
az account set --subscription $SUBSCRIPTION_ID

echo "Creating resource group: $RESOURCE_GROUP_NAME in region: $REGION with owner tag: $OWNER_TAG_VALUE..."
# Create a resource group with the random name in the provided region and set the OWNER tag
...
az vm extension set \
    ...
    --version 2.0 \
    --settings '{"script": "cat ./clone-build-run.sh | bash -s > /tmp/run_output.log"}'
