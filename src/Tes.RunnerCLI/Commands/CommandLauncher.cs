...
namespace Tes.RunnerCLI.Commands
{
    ...
    public class CommandLauncher
    {
        ...
        public static async Task LaunchesExecutorCommandAsSubProcessAsync(Runner.Models.NodeTask nodeTask, FileInfo file, string apiVersion, Uri dockerUri, int selector)
        {
            ...
            try
            {
                var processLauncher = await ProcessLauncher.CreateLauncherAsync(
                    nodeTask,
                    // Maintain format with TesApi.Web.Events.RunnerEventsProcessor.GetMessageBatchStateAsync+GetProcessLogsAsync()
                    logNamePrefix: $"{CommandFactory.ExecutorCommandName}-{selector:D3}",
                    apiVersion: apiVersion);
