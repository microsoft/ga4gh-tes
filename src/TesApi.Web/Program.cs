...
namespace TesApi.Web
{
    ...
    public class Program
    {
        ...
        {
            var AzureName = Environment.GetEnvironmentVariable("AzureName");
            if (string.IsNullOrEmpty(AzureName))
            {
                AzureName = "AzureGlobalCloud";
            }
            ...
            .ConfigureLogging((context, logging) =>
            {
                try
                {
                    if (context.HostingEnvironment.IsProduction())
                    {
                        ...
                        var instrumentationKey = AzureProxy.GetAppInsightsInstrumentationKeyAsync(AzureEnvironment.FromName(AzureName), applicationInsightsAccountName).Result;
