...
namespace Tes.Runner.Docker
{
    public class DockerExecutor
    {
        ...
        public virtual async Task<ContainerExecutionResult> RunOnContainerAsync(string? imageName, string? tag, List<string>? commandsToExecute, List<string>? volumeBindings, string? workingDir)
        {
            ...
            await DeleteImageByNameAsync(imageName);
        ...
        private async Task DeleteImageByNameAsync(string imageName)
        {
            try
            {
                ...
                logger.LogInformation($"Deleted Docker image with name: {imageName}");
            }
            catch (Exception e)
            {
                logger.LogError(e, $"Failed to delete image {imageName}. Error: {e.Message}");
                throw;
            }
        }
